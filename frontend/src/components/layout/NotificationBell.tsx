...
export function NotificationBell() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute right-0 z-40 mt-2 w-80 overflow-hidden rounded-2xl border border-border bg-card shadow-popover">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <span className="text-sm font-medium text-foreground">Notifications</span>
            {unread > 0 && (
              <Button variant="ghost" size="sm" className="h-auto rounded-full px-2 py-1 text-xs" onClick={() => markRead.mutate(undefined)}>
                Mark all read
              </Button>
            )}
          </div>
